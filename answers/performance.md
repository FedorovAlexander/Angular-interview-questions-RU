...
Web Worker позволяет выполнять скрипт отдельным процессом в фоновом режиме. Web Worker-ы могут создавать другие Web Worker-ы и так далее. Общение между главным процессом и Web Worker-ами осуществляется с помощью сообщений.
...
Zone.js - это библиотека, которая позволяет следить за асинхронными событиями в Angular. Zone.js создает зону для каждого компонента и следит за асинхронными событиями в этой зоне.
Zone.js используется для запуска change detection в Angular. Zone.js создает зону для каждого компонента и следит за асинхронными событиями в этой зоне.
